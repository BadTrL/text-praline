...
"""
Focused evaluation script to compare RAW vs Praline Full vs Praline Light.
This script runs the controlled experiment as specified in the requirements.
"""
...
DATASETS_DIR = BASE_DIR / "rag_testing" / "datasets"
RESULTS_DIR = DATASETS_DIR / "light_experiment"
...
def make_embedding_function():
    """Make embedding function with fallback."""
    try:
        embed_fn = SentenceTransformerEmbeddingFunction(model_name=EMBED_MODEL)
        _ = embed_fn(["healthcheck"])
        return embed_fn, f"sentence-transformers:{EMBED_MODEL}"
    except Exception:
        # Fallback to hash embedding if sentence transformers fail
...
def run_focused_evaluation():
    ...
    # Use raw variant for QA generation (as in original script)
